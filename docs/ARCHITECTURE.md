...
> For the full decision-pipeline reference (context controls, fallback chains, image-aware escalation, Google thinking tool continuation, context thresholds and auto-compaction, and thinking control), see [How Routing Works](../README.md#how-routing-works) in the README.
...
- **`extensions/provider.ts`**: Implements the `router` provider, delegation/retry loop, model-slug cooldown checks, and context fallback/compaction marking.
- **`extensions/routing.ts`**: Core decision logic, classifier, gating, and classifier-slug cooldown handling.
...
- **`extensions/failover.ts`**: Parses rate-limit failures, computes bounded retry cooldowns, and tracks suspended model slugs.
- **`extensions/compaction.ts`**: Defers and awaits one deferred Pi compaction at the settled-session boundary.
...
session_start / model_select / turn_end (index.ts)
      ...
      provider.ts streamSimple
            ...
            ├─→ Model-slug cooldown check
            ├─→ Context threshold check/response-full detection (mark settled compaction when needed)
            ├─→ Delegate to target model
            └─→ Fallback chain on failure (suspend slug on rate limit)
            │
            ▼
...
>
> **Not persisted**: rate-limit cooldowns (`modelCooldowns`) are intentionally in-memory, per-process state — short-lived (≤60s) by nature, shared across branches, reset on restart.
